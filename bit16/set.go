...
// Set is fixed-width bitset able to store non-negative `int` values up to 15. The zero value is empty and ready to use.
type Set uint16
...
func More(value int) Set { return Set(^uint16(1<<(value+1) - 1)) }
...
func (s Set) Min() int {
	switch val := bits.TrailingZeros16(uint16(s)); val {
	case Cap:
		return -1
	default:
		return val
...
func (s Set) Max() int {
	switch val := bits.LeadingZeros16(uint16(s)); val {
	case Cap:
		return -1
	default:
		return Max - val
...
func (s Set) Compare(other Set) int {
	return cmp.Compare(bits.Reverse16(uint16(s)), bits.Reverse16(uint16(other)))
}
