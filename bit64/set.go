...
// Set is fixed-width bitset able to store non-negative `int` values up to 63. The zero value is empty and ready to use.
type Set uint64
...
func More(value int) Set { return Set(^uint64(1<<(value+1) - 1)) }
...
func (s Set) Min() int {
	switch val := bits.TrailingZeros64(uint64(s)); val {
	case Cap:
		return -1
	default:
		return val
...
func (s Set) Max() int {
	switch val := bits.LeadingZeros64(uint64(s)); val {
	case Cap:
		return -1
	default:
		return Max - val
...
func (s Set) Compare(other Set) int {
	return cmp.Compare(bits.Reverse64(uint64(s)), bits.Reverse64(uint64(other)))
}
